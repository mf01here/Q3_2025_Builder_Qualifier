...
fn airdrop() {
    // Import our keypair
    let keypair = read_keypair_file("dev-wallet.json").expect("Couldn't find wallet file"); // we'll establish a connection to Solana devnet using the const we defined above
...
fn transfer_sol() {
    ...
    //destination (Turbin3) address
    let to_pubkey = Pubkey::from_str("BiN1HLh9L1NiA39CiKnmxCf6sRraiLwsmSZQjwouS4dS").unwrap();
    //Devnet RPC client
...
fn enroll() {
    ...
    let instruction = Instruction { program_id: turbin3_prereq_program, accounts, data };
